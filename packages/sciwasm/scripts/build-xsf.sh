...
echo "XSF WASM Build"
echo "=============="
echo "Source: $SRC_DIR/special, $SRC_DIR/xsf"
echo "Output: $OUT_DIR"
echo ""
...
for cfile in "$SRC_DIR/special/comb.c"; do
    if [ -f "$cfile" ]; then
...
echo ""
echo "Compiling C++ sources..."
for cppfile in "$SRC_DIR/special/gamma.cpp"; do
    if [ -f "$cppfile" ]; then
        base=$(basename "$cppfile" .cpp)
        obj="$OBJ_DIR/${base}.o"
        echo "  Compiling: $(basename "$cppfile")"
        emcc -c "$cppfile" -o "$obj" -std=c++17 -I"$SRC_DIR" -I"$SRC_DIR/xsf" -O2
        XSF_OBJS+=("$obj")
    fi
done
