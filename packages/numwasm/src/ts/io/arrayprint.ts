 ...
 * //  [4. 5. 6.]]
 */
export function array2string(arr: NDArray, options: PrintOptions = {}): string {
 ...
 * //        [4., 5., 6.]], dtype=float64)
 */
export function arrayRepr(arr: NDArray, options: PrintOptions = {}): string {
  const opts = { ...currentOptions, ...options };
  const prefix = "array(";
  const suffix = ")";
 ...
 */
export function arrayStr(arr: NDArray, options: PrintOptions = {}): string {
...
function getSubarray(
  ...
  const firstStride =
    newShape.length > 0 ? newShape.reduce((a, b) => a * b, 1) : 1;
