...
import { FlatIterator } from "../_lib/iterators.js";
import {
  type IndexElement,
  expandEllipsis,
  buildIndexSpecs,
...
export class NDArray {
  ...
  static async ones(
    ...
    return NDArray.full(shape, 1.0, options);
  }
  ...
  static async fromArray(
    ...
    const expectedSize =
      inferredShape.length === 0 ? 1 : inferredShape.reduce((a, b) => a * b, 1);
  ...
  static async fromTypedArray(
    ...
    const inferredDtype =
      dtype ?? NDArray._inferDtypeFromTypedArray(typedArray);
  ...
  static async arange(
    ...
    const ptr = module._ndarray_arange(start, end, step, dtype);

    if (ptr === 0) {
      throw new Error("Failed to create arange array");
    }
  ...
  static async linspace(
    ...
    const ptr = module._ndarray_linspace(
      start,
      stop,
      num,
      endpoint ? 1 : 0,
    ...
    if (ptr === 0) {
      throw new Error("Failed to create linspace array");
    }
  ...
  static async geomspace(
    ...
    const data = (linArr.toArray() as number[]).map(
      (x) => sign * Math.pow(10, x),
    );
    linArr.dispose();
    return NDArray.fromArray(data, [num], options);
  }
  ...
  static async eye(
    ...
    const ptr = module._ndarray_eye(N, cols, k, dtype);

    if (ptr === 0) {
      throw new Error("Failed to create eye array");
    }
  ...
  static async tri(
    ...
    const ptr = module._ndarray_tri(N, cols, k, dtype);

    if (ptr === 0) {
      throw new Error("Failed to create tri array");
    }
  ...
  static async tril(arr: NDArray, k: number = 0): Promise<NDArray> {
    ...
    const ptr = arr._module._ndarray_tril(arr._ptr, k);

    if (ptr === 0) {
      throw new Error("Failed to create tril array");
    }

    return new NDArray(ptr, arr._module);
  }
  ...
  static async triu(arr: NDArray, k: number = 0): Promise<NDArray> {
    ...
    const ptr = arr._module._ndarray_triu(arr._ptr, k);

    if (ptr === 0) {
      throw new Error("Failed to create triu array");
    }
  ...
  static async vander(
    ...
    if (x.ndim !== 1) {
      throw new Error("Input must be 1D array");
    }

    const numCols = N ?? x.size;
    const ptr = x._module._ndarray_vander(x._ptr, numCols, increasing ? 1 : 0);

    if (ptr === 0) {
      throw new Error("Failed to create Vandermonde matrix");
    }

    return new NDArray(ptr, x._module);
  }
  ...
  get imag(): NDArray {
    ...
    if (this.dtype !== DType.Complex64 && this.dtype !== DType.Complex128) {
      ...
      const ptr = this._module._ndarray_create(
        shape.length,
        shapePtr,
        targetDtype,
      );
  ...
  get(...indices: number[]): number {
    ...
    // Check bounds first
    if (
      !this._module._ndarray_check_bounds(this._ptr, indicesPtr, indices.length)
    ) {
  ...
  getFlat(index: number): number {
    ...
    if (index < 0 || index >= this.size) {
      throw new Error(
        `Flat index ${index} out of bounds for size ${this.size}`,
      );
    }
  ...
  getComplex(...indices: number[]): Complex {
    ...
    if (this.dtype !== DType.Complex64 && this.dtype !== DType.Complex128) {
      throw new Error(
        `getComplex() requires a complex dtype, got ${this.dtypeName}`,
      );
    }
  ...
  setComplex(real: number, imag: number, ...indices: number[]): void {
    ...
    if (this.dtype !== DType.Complex64 && this.dtype !== DType.Complex128) {
      throw new Error(
        `setComplex() requires a complex dtype, got ${this.dtypeName}`,
      );
    }
  ...
  moveaxis(source: number | number[], destination: number | number[]): NDArray {
    ...
    const normSrc = src.map((s) => {
      ...
      if (normalized < 0 || normalized >= ndim) {
        throw new Error(
          `source axis ${s} is out of bounds for array with ${ndim} dimensions`,
        );
      }
      return normalized;
    });
    ...
    const normDst = dst.map((d) => {
      ...
      if (normalized < 0 || normalized >= ndim) {
        throw new Error(
          `destination axis ${d} is out of bounds for array with ${ndim} dimensions`,
        );
      }
      return normalized;
    });
  ...
  slice(indices: IndexElement[]): NDArray {
    ...
    const resultPtr = this._module._ndarray_slice(
      this._ptr,
      specsPtr,
      specs.length,
    );
  ...
  viewDtype(dtype: DType): NDArray {
    ...
    if (resultPtr === 0) {
      throw new Error(
        "Failed to create dtype view: array must be C-contiguous and sizes must be compatible",
      );
    }
