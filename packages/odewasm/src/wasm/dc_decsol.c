/* /workspace/packages/odewasm/reference/stiff/stiff/dc_decsol.f -- translated by f2c (version 20240504).
...
/* ---  B=IDENTITY, JACOBIAN A BANDED MATRIX */
    i__1 = *n;
    for (j = 1; j <= i__1; ++j) {
	i__2 = linal_dcs_1.mbjac;
	for (i__ = 1; i__ <= i__2; ++i__) {
	    e1[i__ + linal_dcs_1.mle + j * e1_dim1] = -fjac[i__ + j * fjac_dim1];
	}
	e1[linal_dcs_1.mdiag + j * e1_dim1] += *fac1;
    }
    decb_(n, lde1, &e1[e1_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &ip1[1], ier);
    return 0;
...
/* ---  B=IDENTITY, JACOBIAN A BANDED MATRIX, SECOND ORDER */
	...
	i__2 = linal_dcs_1.mbjac;
	for (i__ = 1; i__ <= i__2; ++i__) {
	    e1[i__ + linal_dcs_1.mle + j * e1_dim1] = -fjac[i__ + jm1 * fjac_dim1]
		    ;
	}
	e1[linal_dcs_1.mdiag + j * e1_dim1] += *fac1;
    }
L46:
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
	    ...
	    e1[i__ + linal_dcs_1.mle + j * e1_dim1] -= sum;
	}
    }
    decb_(nm1, lde1, &e1[e1_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &ip1[1], ier)
	    ;
    return 0;
...
/* Computing MIN */
	...
	for (i__ = max(i__2,i__3); i__ <= i__4; ++i__) {
	    e1[i__ + j * e1_dim1] += *fac1 * fmas[i__ - j + linal_dcs_1.mbdiag + 
		    j * fmas_dim1];
	}
    }
    dec_(n, lde1, &e1[e1_offset], &ip1[1], ier);
...
/* ---  B IS A BANDED MATRIX, JACOBIAN A BANDED MATRIX */
    i__1 = *n;
    for (j = 1; j <= i__1; ++j) {
	i__3 = linal_dcs_1.mbjac;
	for (i__ = 1; i__ <= i__3; ++i__) {
	    e1[i__ + linal_dcs_1.mle + j * e1_dim1] = -fjac[i__ + j * fjac_dim1];
	}
	i__3 = linal_dcs_1.mbb;
	for (i__ = 1; i__ <= i__3; ++i__) {
	    ib = i__ + linal_dcs_1.mdiff;
...
/* ---  B=IDENTITY, JACOBIAN A BANDED MATRIX */
	...
	i__2 = linal_dcs_1.mbjac;
	for (i__ = 1; i__ <= i__2; ++i__) {
	    imle = i__ + linal_dcs_1.mle;
	    e2r[imle + j * e2r_dim1] = -fjac[i__ + j * fjac_dim1];
	    e2i[imle + j * e2i_dim1] = 0.;
	}
	e2r[linal_dcs_1.mdiag + j * e2r_dim1] += *alphn;
	e2i[linal_dcs_1.mdiag + j * e2i_dim1] = *betan;
    }
    decbc_(n, lde1, &e2r[e2r_offset], &e2i[e2i_offset], &linal_dcs_1.mle, &
	    linal_dcs_1.mue, &ip2[1], ier);
    return 0;
...
/* ---  B=IDENTITY, JACOBIAN A BANDED MATRIX, SECOND ORDER */
	...
	i__2 = linal_dcs_1.mbjac;
	for (i__ = 1; i__ <= i__2; ++i__) {
	    e2r[i__ + linal_dcs_1.mle + j * e2r_dim1] = -fjac[i__ + jm1 * 
		    fjac_dim1];
	    e2i[i__ + linal_dcs_1.mle + j * e2i_dim1] = 0.;
	}
	e2r[linal_dcs_1.mdiag + j * e2r_dim1] += *alphn;
	e2i[linal_dcs_1.mdiag + j * e2i_dim1] += *betan;
    }
...
/* Computing 2nd power */
    ...
    decbc_(nm1, lde1, &e2r[e2r_offset], &e2i[e2i_offset], &linal_dcs_1.mle, &
	    linal_dcs_1.mue, &ip2[1], ier);
    return 0;
...
/* Computing MIN */
	...
	i__4 = min(i__5,i__6);
	for (i__ = max(i__2,i__3); i__ <= i__4; ++i__) {
	    bb = fmas[i__ - j + linal_dcs_1.mbdiag + j * fmas_dim1];
...
/* Computing MIN */
	...
	i__3 = min(i__5,i__6);
	for (i__ = max(i__4,i__2); i__ <= i__3; ++i__) {
	    ffma = fmas[i__ - j + linal_dcs_1.mbdiag + j * fmas_dim1];
...
/* Computing MIN */
	i__5 = linal_dcs_1.mbb, i__6 = *mumas + 1 - j + *n;
	i__2 = min(i__5,i__6);
...
/* ---  B=IDENTITY, JACOBIAN A BANDED MATRIX */
    ...
    solb_(n, lde1, &e1[e1_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &z1[1], &ip1[1]
	    );
    return 0;
...
/* Computing MIN */
    ...
    solb_(nm1, lde1, &e1[e1_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &z1[*m1 + 1],
	     &ip1[1]);
    goto L49;
...
/* Computing MIN */
	...
	for (j = max(i__4,i__2); j <= i__3; ++j) {
	    s1 -= fmas[i__ - j + linal_dcs_1.mbdiag + j * fmas_dim1] * f1[j];
	}
	z1[i__] += s1 * *fac1;
    }
    sol_(n, lde1, &e1[e1_offset], &z1[1], &ip1[1]);
...
/* Computing MIN */
	...
	for (j = max(i__3,i__4); j <= i__2; ++j) {
	    s1 -= fmas[i__ - j + linal_dcs_1.mbdiag + j * fmas_dim1] * f1[j + *m1]
		    ;
	}
	z1[im1] += s1 * *fac1;
    }
...
/* ---  B=IDENTITY, JACOBIAN A BANDED MATRIX */
    ...
    solbc_(n, lde1, &e2r[e2r_offset], &e2i[e2i_offset], &linal_dcs_1.mle, &
	    linal_dcs_1.mue, &z2[1], &z3[1], &ip2[1]);
    return 0;
...
/* Computing MIN */
    ...
    solbc_(nm1, lde1, &e2r[e2r_offset], &e2i[e2i_offset], &linal_dcs_1.mle, &
	    linal_dcs_1.mue, &z2[*m1 + 1], &z3[*m1 + 1], &ip2[1]);
    goto L49;
...
/* ------  B=IDENTITY, JACOBIAN A BANDED MATRIX */
    ...
    solb_(n, lde1, &e1[e1_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &cont[1], &ip1[
	    1]);
    goto L77;
...
/* Computing MIN */
    ...
    solb_(nm1, lde1, &e1[e1_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &cont[*m1 + 
	    1], &ip1[1]);
    for (i__ = *m1; i__ >= 1; --i__) {
	cont[i__] = (cont[i__] + cont[*m2 + i__]) / *fac1;
    }
...
/* Computing MIN */
	...
	i__3 = min(i__5,i__6);
	for (j = max(i__4,i__2); j <= i__3; ++j) {
	    sum += fmas[i__ - j + linal_dcs_1.mbdiag + j * fmas_dim1] * f1[j];
	}
	f2[i__] = sum;
	cont[i__] = sum + y0[i__];
    }
...
L32:
	solb_(n, lde1, &e1[e1_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &cont[1], &
		ip1[1]);
	goto L88;
...
/* Computing MIN */
	...
	solb_(nm1, lde1, &e1[e1_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &cont[*
		m1 + 1], &ip1[1]);
	for (i__ = *m1; i__ >= 1; --i__) {
	    cont[i__] = (cont[i__] + cont[*m2 + i__]) / *fac1;
	}
...
/* Computing MIN */
	...
	i__3 = min(i__5,i__6);
	for (j = max(i__4,i__2); j <= i__3; ++j) {
	    sum += fmas[i__ - j + linal_dcs_1.mbdiag + j * fmas_dim1] * ff[j];
	}
	ff[i__ + *n] = sum;
	cont[i__] = sum + y0[i__];
    }
...
/* ---  B=IDENTITY, JACOBIAN A BANDED MATRIX */
    ...
    solb_(n, lde, &e[e_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &ak[1], &ip[1]);
    return 0;
...
/* Computing MIN */
    ...
    solb_(nm1, lde, &e[e_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &ak[*m1 + 1], &
	    ip[1]);
    for (i__ = *m1; i__ >= 1; --i__) {
	ak[i__] = (ak[i__] + ak[*m2 + i__]) / *fac1;
    }
...
/* Computing MIN */
	    ...
	    i__3 = min(i__5,i__6);
	    for (j = max(i__4,i__2); j <= i__3; ++j) {
		sum += fmas[i__ - j + linal_dcs_1.mbdiag + j * fmas_dim1] * ynew[
			j];
	    }
	    ak[i__] += sum;
	}
...
L2:
/* ---  B=IDENTITY, JACOBIAN A BANDED MATRIX */
    solb_(n, lde, &e[e_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &del[1], &ip[1]);
    return 0;
...
/* Computing MIN */
    ...
    solb_(nm1, lde, &e[e_offset], &linal_dcs_1.mle, &linal_dcs_1.mue, &del[*m1 + 1], &
	    ip[1]);
    for (i__ = *m1; i__ >= 1; --i__) {
	del[i__] = (del[i__] + del[*m2 + i__]) / *fac1;
    }
