/* ../../reference/nonstiff/nonstiff/odex.f -- translated by f2c (version 20240504).
...
#define conodx_1 (conodx_._1)
#define conodx_2 (conodx_._2)
...
/* Subroutine */ int odex_(integer *n, U_fp fcn, doublereal *x, doublereal *y,
	 doublereal *xend, doublereal *h__, doublereal *rtol, doublereal *
	atol, integer *itol, U_fp solout, integer *iout, doublereal *work, 
	integer *lwork, integer *iwork, integer *liwork, doublereal *rpar, 
	integer *ipar, integer *idid)
{
    ...
    extern /* Subroutine */ int odxcor_(integer *, U_fp, doublereal *, 
...
/* Computing MAX */
    ...
    ncom = max(i__1,i__2);
    odxcor_(n, (U_fp)fcn, x, &y[1], xend, &hmax, h__, &rtol[1], &atol[1], 
	    itol, &km, (U_fp)solout, iout, idid, &nmax, &uround, &work[iedy], 
	    ...
	    &fac2, &fac3, &fac4, &iderr, &work[iefac], &mudif, &nrd, &rpar[1],
	     &ipar[1], &nfcn, &nstep, &naccpt, &nrejct);
    iwork[17] = nfcn;
    iwork[18] = nstep;
    iwork[19] = naccpt;
    iwork[20] = nrejct;
...
/* Subroutine */ int odxcor_(integer *n, S_fp fcn, doublereal *x, doublereal *
	y, doublereal *xend, doublereal *hmax, doublereal *h__, doublereal *
	rtol, doublereal *atol, integer *itol, integer *km, S_fp solout, 
	...
	doublereal *errfac, integer *mudif, integer *nrd, doublereal *rpar, 
	integer *ipar, integer *nfcn, integer *nstep, integer *naccpt, 
	integer *nrejct)
{
    /* Format strings */
    ...
    integer kmi, kln;
    doublereal err;
    integer krn, ipt, kbeg, lbeg, lend, ncon;
    logical last;
    doublereal prod;
    logical atov;
    doublereal xold;
    ...
    integer njadd;
    doublereal facnj;
    extern /* Subroutine */ int midex_(integer *, doublereal *, doublereal *, 
    ...
    extern /* Subroutine */ int interp_(integer *, doublereal *, integer *);
    doublereal errint;

    /* Fortran I/O blocks */
    static cilist io___88 = { 0, 6, 0, fmt_979, 0 };
...
/* Computing MAX */
/* Computing MIN */
    i__3 = *km - 1, i__4 = (integer) (-d_lg10(&rtol[1]) * .6 + 1.5);
...
/* L9: */
	...
	xold = *x;
	i__1 = *naccpt + 1;
	(*solout)(&i__1, &xold, x, &y[1], n, &dens[1], &ncon, &icomp[1], nrd, 
		&rpar[1], &ipar[1], &irtrn);
	if (irtrn < 0) {
	    goto L120;
	}
...
/* Computing MIN */
	...
	*h__ = *xend - *x;
    ...
    if (*nstep == 0 || *iout != 2) {
	(*fcn)(n, x, &y[1], &dz[1], &rpar[1], &ipar[1]);
    }
    ++(*nfcn);
/* --- THE FIRST AND LAST STEP */
	...
	for (j = 1; j <= i__1; ++j) {
	    kc = j;
	    midex_(&j, x, &y[1], h__, hmax, n, (S_fp)fcn, &dy[1], &yh1[1], &
		    ...
		    mstab, jstab, &errold, &fsafe[fsafe_offset], lfsafe, iout,
		     &ipt, &ysafe[ysafe_offset], &icomp[1], nrd, &rpar[1], &
		    ipar[1], nfcn);
	    if (atov) {
		goto L10;
	    }
...
L30:
    ...
    i__1 = kc;
    for (j = 1; j <= i__1; ++j) {
	midex_(&j, x, &y[1], h__, hmax, n, (S_fp)fcn, &dy[1], &yh1[1], &yh2[1]
		...
		, &icomp[1], nrd, &rpar[1], &ipar[1], nfcn);
	if (atov) {
	    goto L10;
	}
/* L40: */
...
L50:
    midex_(&k, x, &y[1], h__, hmax, n, (S_fp)fcn, &dy[1], &yh1[1], &yh2[1], &
...
/* Computing 2nd power */
    ...
    kc = k + 1;
    midex_(&kc, x, &y[1], h__, hmax, n, (S_fp)fcn, &dy[1], &yh1[1], &yh2[1], &
	    dz[1], &t[t_offset], &nj[1], &hh[1], &w[1], &err, &fac, &a[1], 
	    safe1, uround, fac1, fac2, safe2, &scal[1], &atov, safe3, &reject,
	     km, &rtol[1], &atol[1], itol, mstab, jstab, &errold, &fsafe[
	    fsafe_offset], lfsafe, iout, &ipt, &ysafe[ysafe_offset], &icomp[1]
...
/* L478: */
	...
	(*fcn)(n, x, &yh1[1], &yh2[1], &rpar[1], &ipar[1]);
	krn = *nrd * 3;
	i__2 = *nrd;
	for (i__ = 1; i__ <= i__2; ++i__) {
/* L274: */
...
L180:
	...
	interp_(nrd, &dens[1], &conodx_1.kmit);
...
/* L187: */
/* Computing 2nd power */
		d__1 = dens[(conodx_1.kmit + 4) * *nrd + i__] / scal[icomp[
			i__]];
		errint += d__1 * d__1;
	    }
	    errint = sqrt(errint / *nrd) * errfac[conodx_1.kmit];
/* Computing MAX */
	    d__2 = 1. / (conodx_1.kmit + 4);
...
/* L70: */
    ...
    if (*iout >= 1) {
	i__2 = *naccpt + 1;
	(*solout)(&i__2, &xold, x, &y[1], n, &dens[1], ncom, &icomp[1], nrd, &
		rpar[1], &ipar[1], &irtrn);
	if (irtrn < 0) {
	    goto L120;
	}
...
/* Subroutine */ int midex_(integer *j, doublereal *x, doublereal *y, 
	...
	icomp, integer *nrd, doublereal *rpar, integer *ipar, integer *nfcn)
{
    /* System generated locals */
    integer t_dim1, t_offset, fsafe_dim1, fsafe_offset, ysafe_dim1, 
...
/* L31: */
	...
	d__1 = *x + hj * mm;
	(*fcn)(n, &d__1, &yh2[1], &dy[1], &rpar[1], &ipar[1]);
...
/* --- COMPUTE OPTIMAL STEPSIZES */
    expo = 1. / ((*j << 1) - 1);
    facmin = pow_dd(fac1, &expo);
/* Computing MIN */
/* Computing MAX */
    d__5 = *err / *safe1;
    d__3 = facmin, d__4 = pow_dd(&d__5, &expo) / *safe2;
...
/* Subroutine */ int interp_(integer *n, doublereal *y, integer *imit)
{
    /* System generated locals */
    integer i__1, i__2;
...
doublereal contex_(integer *ii, doublereal *x, doublereal *y, integer *ncon, 
	integer *icomp, integer *n)
{
    /* System generated locals */
    integer i__1;
    ...
    /* Local variables */
    integer i__, j, im;
    doublereal theta, theta1, thetah, phthet;
...
/* ----- COMPUTE THE INTERPOLATED VALUE */
    theta = (*x - conodx_2.xold) / conodx_2.h__;
    theta1 = 1. - theta;
    phthet = y[i__] + theta * (y[*n + i__] + theta1 * (y[(*n << 1) + i__] * 
	    theta + y[*n * 3 + i__] * theta1));
    if (conodx_2.imit < 0) {
	ret_val = phthet;
	return ret_val;
    }
    thetah = theta - .5;
    ret_val = y[*n * (conodx_2.imit + 4) + i__];
    for (im = conodx_2.imit; im >= 1; --im) {
/* L70: */
	ret_val = y[*n * (im + 3) + i__] + ret_val * thetah / im;
    }
/* Computing 2nd power */
    d__1 = theta * theta1;
    ret_val = phthet + d__1 * d__1 * ret_val;
    return ret_val;
} /* contex_ */
