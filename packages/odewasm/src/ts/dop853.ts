 ...
 * DOPRI5 when high accuracy is required or the solution is smooth.
 */
...
import { STATUS_MESSAGES } from "./types.js";
 ...
 *
 * DOP853 is an explicit Runge-Kutta method of order 8 with embedded
 * methods of orders 5 and 3 for error estimation. More efficient than
 * DOPRI5/RK45 when high accuracy is required.
 *
 ...
 * @example
 * ```ts
 * import { dop853 } from 'odewasm';
 *
 * // Harmonic oscillator: y'' = -y => [y, y'] => [y', -y]
 * const result = await dop853(
 *   (t, [y, v]) => [v, -y],
 *   [0, 10],
 *   [1, 0],  // y(0) = 1, y'(0) = 0
 *   { rtol: 1e-10 }
 ...
 * ```
 */
export async function dop853(
  fun: ODEFunction,
  t_span: [number, number],
  y0: number[],
  options?: Dop853Options,
  ...
  const { idid, nfev } = await solveDop853Internal(
    Module,
    n,
    t0,
    y0,
...
/**
 * Internal implementation of DOP853 solver.
 */
async function solveDop853Internal(
  Module: ODEModule,
  n: number,
  t0: number,
  y0: number[],
  ...
  const lwork = Module._wasm_dop853_work_size(n, nrdens);
  const liwork = Module._wasm_dop853_iwork_size(nrdens);
