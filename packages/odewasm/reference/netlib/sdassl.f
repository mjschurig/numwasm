...
C-----------------------------------------------------------------------
C NOTE:  Users of this solver, SDASSL, are encouraged to use the
C solver SDASPK instead.  SDASPK has a much improved initial condition
C calculation algorithm.  In addition, SDASPK includes iterative
C (Krylov) methods for the linear systems that arise, in addition to
C the direct (dense/banded) methods in SDASSL.
C-----------------------------------------------------------------------
...
C *Usage:
C        REAL  A, R1MACH
C        A = R1MACH(idummy)  [The argument is ignored.]
C
...
C     number u such that  1.0 + u .ne. 1.0.  This is computed by R1MACH
...
C     Routine to force normal storing of A + B, for R1MACH.
      REAL A, B, C
      C = A + B
      RETURN
      END
