...
c
c     data twoeps, u26/4.4e-16, 5.72e-15/                               ***
      twoeps = 2.*r1mach(4)                                             ***
      u26 = 13.*twoeps                                                  ***
c
c
...
c
      ...
      if ((relerr .lt. 0.0e0)  .or.  (abserr .lt. 0.0e0)) go to 10
      mflag=iabs(iflag)
...
c     iflag = +2 or -2
      ...
      if ((kflag .eq. 5)  .and.  (abserr .eq. 0.0e0)) go to 30
...
c     iflag = 3,4,5,6,7 or 8
   25 if (iflag .eq. 3) go to 45
      ...
      if ((iflag .eq. 5) .and. (abserr .gt. 0.0e0)) go to 45
c
...
c
   65 init=1
      ...
      do 70 k=1,neqn
        ...
        toln=tol
        ypk=abs(yp(k))
        if (ypk*h**5 .gt. tol) h=(tol/ypk)**0.2e0
   70 continue
      if (toln .le. 0.0e0) h=0.0e0
...
c     output points
c
      if (abs(h) .ge. 2.0e0*abs(dt)) kop=kop+1
...
c
      dt=tout-t
      if (abs(dt) .ge. 2.0e0*abs(h)) go to 200
...
c
      ...
      do 250 k=1,neqn
        ...
        if (et .gt. 0.0e0) go to 240
c
c       inappropriate error tolerance
        iflag=5
        return
c
  240   ee=abs((-2090.0e0*yp(k)+(21970.0e0*f3(k)-15048.0e0*f4(k)))+
     1                        (22528.0e0*f2(k)-27360.0e0*f5(k)))
  250   eeoet=amax1(eeoet,ee/et)
c
      esttol=abs(h)*eeoet*scale/752400.0e0
c
      if (esttol .le. 1.0e0) go to 260
c
c
c     unsuccessful step
...
c
      ...
      output= .false.
      s=0.1e0
      if (esttol .lt. 59049.0e0) s=0.9e0/esttol**0.2e0
      h=s*h
...
c
      s=5.0e0
      if (esttol .gt. 1.889568e-4) s=0.9e0/esttol**0.2e0
...
c
      ch=3.0e0*h/32.0e0
      do 222 k=1,neqn
  222   f5(k)=y(k)+ch*(yp(k)+3.0e0*f1(k))
      call f(t+3.0e0*h/8.0e0,f5,f2)
c
      ch=h/2197.0e0
      do 223 k=1,neqn
  223   f5(k)=y(k)+ch*(1932.0e0*yp(k)+(7296.0e0*f2(k)-7200.0e0*f1(k)))
      call f(t+12.0e0*h/13.0e0,f5,f3)
c
      ch=h/4104.0e0
      do 224 k=1,neqn
  224   f5(k)=y(k)+ch*((8341.0e0*yp(k)-845.0e0*f3(k))+
     1                            (29440.0e0*f2(k)-32832.0e0*f1(k)))
      call f(t+h,f5,f4)
c
      ch=h/20520.0e0
      do 225 k=1,neqn
  225   f1(k)=y(k)+ch*((-6080.0e0*yp(k)+(9295.0e0*f3(k)-
     1         5643.0e0*f4(k)))+(41040.0e0*f1(k)-28352.0e0*f2(k)))
...
c
      ch=h/7618050.0e0
      do 230 k=1,neqn
  230   s(k)=y(k)+ch*((902880.0e0*yp(k)+(3855735.0e0*f3(k)-
     1        1371249.0e0*f4(k)))+(3953664.0e0*f2(k)+
     2        277020.0e0*f5(k)))
c
      return
      end
