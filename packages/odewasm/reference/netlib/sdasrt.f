*DECK SDASRT
      SUBROUTINE SDASRT (RES,NEQ,T,Y,YPRIME,TOUT,
     *  INFO,RTOL,ATOL,IDID,RWORK,LRW,IWORK,LIW,RPAR,IPAR,JAC,
     *  G,NG,JROOT)
C-----------------------------------------------------------------------
C NOTE:  Users of this solver, SDASRT, are encouraged to use the
C solver SDASKR instead.  SDASKR has a much improved initial condition
C calculation algorithm, and also has improvements in the rootfinding
C algorithm.  In addition, SDASKR includes iterative (Krylov) methods
C for the linear systems that arise, in addition to the direct
C (dense/banded) methods in SDASRT.
C-----------------------------------------------------------------------
C***BEGIN PROLOGUE  SDASRT
...
C     *   RWORK(LRW), RPAR
C
C      CALL SDASRT (RES, NEQ, T, Y, YPRIME, TOUT, INFO, RTOL, ATOL,
...
C
C  Subroutine SDASRT uses the backward differentiation formulas of
...
C  capability.  If SDASRT detects a sign-change in G(T,Y), then
C  it will return the intermediate value of T and Y for which
C  G(T,Y) = 0.
C
C  ---------INPUT-WHAT TO DO ON THE FIRST CALL TO SDASRT---------------
C
C
...
C         statement in your program that calls SDASRT.
...
C         is illegal, and SDASRT will try to solve the problem
C         without getting IRES = -1. If IRES = -2, SDASRT
C         will return control to the calling program
C         with IDID = -11.
C
C         RPAR and IPAR are real and integer parameter arrays which
C         you can use for communication between your calling program
C         and subroutine RES. They are not altered by SDASRT. If you
...
C            dimensioned of length 15, though SDASRT uses
...
C
C       INFO(6) - SDASRT will perform much better if the matrix of
C              partial derivatives, DG/DY + CJ*DG/DYPRIME,
C              (here CJ is a scalar determined by SDASRT)
...
C                   No - Set INFO(10)=1
C
C        INFO(11) --SDASRT normally requires the initial T,
...
C               derivative precisely, you can let SDASRT try
C               to compute it.
...
C               between your program that calls SDASRT and the
C               RES subroutine (and the JAC subroutine). They are not
C               altered by SDASRT. If you do not need RPAR or IPAR,
...
C               SDASRT. You must dimension Y, YPRIME and PD
C               in JAC.
C
...
C               SDASRT will find roots of the G(I) of odd multiplicity
...
C               G(I) due to roundoff and integration error, SDASRT
...
C               the input to SDASRT should nevertheless allow
C               integration to a point slightly past that ROOT, so
C               that SDASRT can locate the root by interpolation.
C
...
C  SDASRT uses a weighted norm SDANRM to measure the size
...
C  weights.  A SDANRM routine has been included with SDASRT
...
C
C
C------OUTPUT-AFTER ANY RETURN FROM SDASRT----
C
...
C
C           IDID = -6 -- SDASRT had repeated error test
...
C                   calling program.
C
C           IDID =-12 -- SDASRT failed to compute the initial
C                   YPRIME.
C
C
C
...
C                  last attempted step in SDASRT. A singularity in the
...
C                  on the last attempted step in SDASRT. An inaccurate
...
C                  the integration.
C
C    IDID =-10, SDASRT had multiple convergence test failures
C                  because IRES was equal to minus one.
...
C
C    IDID =-12, SDASRT failed to compute the initial YPRIME.
...
C
C***ROUTINES CALLED  SDASTP,SDAINI,SDANRM,SDAWTS,SDATRP,SRCHEK,SROOTS,
C                    XERRWV,R1MACH
C***END PROLOGUE  SDASRT
C
C**End
C
...
C     SET POINTERS INTO RWORK
     ...
     *  LDELTA=51)
C
C***FIRST EXECUTABLE STATEMENT  SDASRT
...
C
      ...
      IWORK(LIRFND) = 0
      RWORK(LPSI)=H
      RWORK(LPSI+1)=2.0E0*H
      IWORK(LKOLD)=1
      IF(NG .EQ. 0) GO TO 390
      CALL SRCHEK(1,G,NG,NEQ,T,TOUT,Y,RWORK(LE),RWORK(LPHI),
...
C
C     Check for a root in the interval (T0,TN], unless SDASRT
C     did not have to initialize YPRIME.
C
      IF(NG .EQ. 0 .OR. INFO(11) .EQ. 0) GO TO 390
      CALL SRCHEK(3,G,NG,NEQ,TN,TOUT,Y,RWORK(LE),RWORK(LPHI),
     *  RWORK(LPSI),IWORK(LKOLD),RWORK(LG0),RWORK(LG1),
     *  RWORK(LGX),JROOT,IRT,RWORK(LROUND),INFO(3),
     *  RWORK,IWORK,RPAR,IPAR)
...
C
      CALL SRCHEK(2,G,NG,NEQ,TN,TOUT,Y,RWORK(LE),RWORK(LPHI),
     *  RWORK(LPSI),IWORK(LKOLD),RWORK(LG0),RWORK(LG1),
     *  RWORK(LGX),JROOT,IRT,RWORK(LROUND),INFO(3),
     *  RWORK,IWORK,RPAR,IPAR)
...
420   IF((TN-T)*H .LE. 0.0E0) GO TO 490
      IF((TN - TOUT)*H .GE. 0.0E0) GO TO 425
...
440   TSTOP = RWORK(LTSTOP)
      ...
      IF((TN - TOUT)*H .GE. 0.0E0) GO TO 445
      CALL SDATRP(TN,TN,Y,YPRIME,NEQ,IWORK(LKOLD),
     *  RWORK(LPHI),RWORK(LPSI))
      T = TN
      IDID = 1
...
C
C--------------------------------------------------------
C     ALL SUCCESSFUL RETURNS FROM SDASRT ARE MADE FROM
C     THIS BLOCK.
C--------------------------------------------------------
C
590   CONTINUE
...
650   MSG = 'DASSL--  AT T (=R1) AND STEPSIZE H (=R2) THE'
      ...
      CALL XERRWV(MSG,48,651,0,0,0,0,0,0.0E0,0.0E0)
      MSG = 'DASSL--  OR WITH ABS(H)=HMIN'
      CALL XERRWV(MSG,28,652,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 690
C
...
660   MSG = 'DASSL--  AT T (=R1) AND STEPSIZE H (=R2) THE'
      ...
      CALL XERRWV(MSG,37,661,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 690
C
...
670   MSG = 'DASSL--  AT T (=R1) AND STEPSIZE H (=R2) THE'
      ...
      CALL XERRWV(MSG,49,671,0,0,0,0,0,0.0E0,0.0E0)
      MSG = 'DASSL--  ERROR TEST FAILED REPEATEDLY.'
      CALL XERRWV(MSG,38,672,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 690
C
...
675   MSG = 'DASSL--  AT T (=R1) AND STEPSIZE H (=R2) THE'
      ...
      CALL XERRWV(MSG,45,676,0,0,0,0,0,0.0E0,0.0E0)
      MSG = 'DASSL--  IRES WAS EQUAL TO MINUS ONE'
      CALL XERRWV(MSG,36,677,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 690
C
...
685   MSG = 'DASSL--  AT T (=R1) AND STEPSIZE H (=R2) THE'
      ...
      CALL XERRWV(MSG,45,686,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 690
690   CONTINUE
      INFO(1)=-1
      T=TN
...
701   MSG = 'DASSL--  SOME ELEMENT OF INFO VECTOR IS NOT ZERO OR ONE'
      CALL XERRWV(MSG,55,1,0,0,0,0,0,0.0E0,0.0E0)
      GO TO 750
...
730   MSG = 'DASSL--  NG (=I1) .LT. 0'
      CALL XERRWV(MSG,24,30,1,1,NG,0,0,0.0E0,0.0E0)
      GO TO 750
732   MSG = 'DASSL--  ONE OR MORE COMPONENTS OF G HAS A ROOT'
      CALL XERRWV(MSG,47,32,1,0,0,0,0,0.0E0,0.0E0)
      MSG = '         TOO NEAR TO THE INITIAL POINT'
      CALL XERRWV(MSG,38,32,1,0,0,0,0,0.0E0,0.0E0)
750   IF(INFO(1).EQ.-1) GO TO 760
      INFO(1)=-1
      IDID=-33
      RETURN
760   MSG = 'DASSL--  REPEATED OCCURRENCES OF ILLEGAL INPUT'
      CALL XERRWV(MSG,46,801,0,0,0,0,0,0.0E0,0.0E0)
770   MSG = 'DASSL--  RUN TERMINATED. APPARENT INFINITE LOOP'
      CALL XERRWV(MSG,47,802,1,0,0,0,0,0.0E0,0.0E0)
      RETURN
C---------- END OF SUBROUTINE SDASRT -----------------------------------
      END
*DECK SRCHEK
      SUBROUTINE SRCHEK (JOB, G, NG, NEQ, TN, TOUT, Y, YP, PHI, PSI,
     *  KOLD, G0, G1, GX, JROOT, IRT, UROUND, INFO3, RWORK, IWORK,
     *  RPAR, IPAR)
C
C***BEGIN PROLOGUE  SRCHEK
C***REFER TO SDASRT
C***ROUTINES CALLED  SDATRP, SROOTS, SCOPY
C***DATE WRITTEN   831001   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
C***END PROLOGUE  SRCHEK
C
      IMPLICIT REAL(A-H,O-Z)
     ...
     *           LT0=41, LTLAST=42, LALPHR=43, LX2=44)
      ...
      REAL TN, TOUT, Y, YP, PHI, PSI, G0, G1, GX, UROUND,
     *  RWORK, RPAR
...
C INPUT FLAG JOB.  IT CALLS SUBROUTINE SROOTS TO LOCATE THE ROOT
...
C ZERO VALUES.----------------------------------------------------------
 100  CONTINUE
      CALL SDATRP(TN,RWORK(LT0),Y,YP,NEQ,KOLD,PHI,PSI)
...
C G HAS A ZERO AT T0.  LOOK AT G AT T + (SMALL INCREMENT). -------------
      TEMP1 = SIGN(HMING,H)
      RWORK(LT0) = RWORK(LT0) + TEMP1
      IF ((RWORK(LT0) - TN)*H .LT. 0.0E0) GO TO 230
      TEMP2 = TEMP1/H
      DO 220 I = 1,NEQ
 220    Y(I) = Y(I) + TEMP2*PHI(I,2)
      GO TO 240
 230  CALL SDATRP (TN, RWORK(LT0), Y, YP, NEQ, KOLD, PHI, PSI)
...
C SET T1 TO TN OR TOUT, WHICHEVER COMES FIRST, AND GET G AT T1. --------
      ...
      IF ((TOUT - TN)*H .GE. 0.0E0) GO TO 310
      T1 = TOUT
      IF ((T1 - RWORK(LT0))*H .LE. 0.0E0) GO TO 390
      CALL SDATRP (TN, T1, Y, YP, NEQ, KOLD, PHI, PSI)
      GO TO 330
 310  T1 = TN
      DO 320 I = 1,NEQ
 320    Y(I) = PHI(I,1)
...
C CALL SROOTS TO SEARCH FOR ROOT IN INTERVAL FROM T0 TO T1. ------------
      JFLAG = 0
 350  CONTINUE
      CALL SROOTS (NG, HMING, JFLAG, RWORK(LT0), T1, G0, G1, GX, X,
...
*DECK SROOTS
     ...
     *                   IMAX, LAST, ALPHA, X2)
C
C***BEGIN PROLOGUE  SROOTS
C***REFER TO SDASRT
C***ROUTINES CALLED  SCOPY
C***DATE WRITTEN   831001   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
C***END PROLOGUE  SROOTS
C
      IMPLICIT REAL(A-H,O-Z)
      INTEGER NG, JFLAG, JROOT, IMAX, LAST
      REAL HMIN, X0, X1, G0, G1, GX, X, ALPHA, X2
